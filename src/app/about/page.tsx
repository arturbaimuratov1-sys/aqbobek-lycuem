...
export default function AboutPage() {
  return (
    <>
              ...
              <ul className="mt-4 space-y-3">
                {site.values.map((v) => (
                  <li key={v} className="flex items-center gap-3 font-display text-lg text-navy-900">
                    <span aria-hidden="true" className="h-1 w-1 bg-steel-600" />
                    {v}
                  </li>
                ))}
              </ul>
      ...
      <section aria-labelledby="about-why" className="border-t border-line bg-mist">
          ...
          <div className="mt-10 grid gap-px border border-line bg-line sm:grid-cols-2 lg:grid-cols-3">
            {whyLyceum.map((item, i) => (
              <Reveal key={item.title} delay={Math.min(i * 0.05, 0.15)} className="bg-white p-8">
                <p aria-hidden="true" className="font-display text-sm text-steel-600">
            ...
            <Reveal className="flex flex-col justify-between bg-navy-900 p-8">
              <div>
                ...
                <p className="mt-2 text-[14.5px] leading-relaxed text-frost-100">
                  Қабылдау шарттарымен танысып, өтініш қалдырыңыз.
                </p>
              </div>
              <Link
                href="/admissions"
                className="mt-6 inline-flex items-center gap-2 text-[15px] font-semibold text-steel-100 underline-offset-4 hover:underline"
              >
